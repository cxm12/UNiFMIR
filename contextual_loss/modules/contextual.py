...
class ContextualLoss(nn.Module):
    ...
    def forward(self, x, y):
        if hasattr(self, 'vgg_model'):
            ...
            # normalization
            if gpu:
                x = x.sub(self.vgg_mean.cuda().detach()).div(self.vgg_std.cuda().detach())
                y = y.sub(self.vgg_mean.cuda().detach()).div(self.vgg_std.cuda().detach())
            else:
