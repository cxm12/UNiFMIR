...
SavePath = '../dataset/train/ER'; % CCPs'; % F-actin'; % Microtubules';% SavePath: directory to save augumented data
DataPath = 'F:/SRdata/train_data/Medical/biosr-ER/ER'; % '../dataset/BioSR/CCPs'; % F-actin'; % Microtubules'; % DataPath: directory of BioSR dataset, eg: '../dataset/BioSR/F-actin';
DataFilter = 'RawSIMData_level_*.mrc'; % DataFilter: raw SIM data in BioSR
GtFilter = 'SIM_gt.mrc'; % GtFilter: GT-SIM data in BioSR

RotFlag = 2; % RotFlag: 0 for no rotatioin, 1 for random rotation, 2 for flipping and random rotaion
SaveRawFlag = 1; % SaveRawFlag: 0 for only saving WF patches, 1 for saving raw SIM image patches

SegX = 128; % SegX: target patch size x
SegY = 128; % SegY: target patch size y
ValidateNum = 200; % 2000; % ValidateNum: number of patch pairs for validation
TotalSegNum = 20000; % TotalSegNum: number of patch pairs for training

bg_cut_prct = 0; % bg_cut_prct: percentile value of gt background, used for some specific data, eg. CCPs
% % Microtubules
% train_cell_list = 1:55; % train_cell_list: cells used for generating training image patch pairs
% train_snr_list = 1:9; % 1:12; % train_snr_list: snr levels used for generating training image patch pairs  
% validate_cell_list = 1:2; % 1:5 validate_cell_list: cells used for generating validation image patch pairs
% validate_snr_list = 1:9; % 1:12; % validate_snr_list: snr levels used for generating validation image patch pairs
% % F-actin
% train_cell_list = 1:51; % train_cell_list: cells used for generating training image patch pairs
% train_snr_list = 1:12; % train_snr_list: snr levels used for generating training image patch pairs  
% validate_cell_list = 1:2; % 1:5 validate_cell_list: cells used for generating validation image patch pairs
% validate_snr_list = 1:12; % validate_snr_list: snr levels used for generating validation image patch pairs

% % CCPs
% train_cell_list = 1:54; % train_cell_list: cells used for generating training image patch pairs
% train_snr_list = 1:9; % train_snr_list: snr levels used for generating training image patch pairs  
% validate_cell_list = 1:2; % 1:5 validate_cell_list: cells used for generating validation image patch pairs
% validate_snr_list = 1:9; % validate_snr_list: snr levels used for generating validation image patch pairs

% % ER
train_cell_list = 1:68; % train_cell_list: cells used for generating training image patch pairs
train_snr_list = 1:6; % train_snr_list: snr levels used for generating training image patch pairs  
validate_cell_list = 0:0; % 1:5 validate_cell_list: cells used for generating validation image patch pairs
validate_snr_list = 1:6; % validate_snr_list: snr levels used for generating validation image patch pairs
