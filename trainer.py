    ...
    def test3DdenoiseInchannel5(self, batch=0, epoch=None, condition=1):
        ...
        if self.args.test_only:
            ...
            for dp in range(0, len(hr), batchstep):
                ...
                # 2.2D norm 0–100
                lrtn = torch.from_numpy(normalize(lrt[:, dp:dp + batchstep, :, :].detach().cpu().numpy(), 0, 100)).to(self.device)
                
                lrtn = torch.transpose(lrtn, 1, 0)  # [batch, 1, h, w]
                a = self.model(lrtn, 0)
            ...
            # 5.3D norm0100 psnr
            sr255 = np.float32(normalize(sr, datamin, datamax, clip=True)) * 255  # [0, 1]
            hr255 = np.float32(normalize(hr, datamin, datamax, clip=True)) * 255  # [0, 1]
