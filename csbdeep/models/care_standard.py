...
class CARE(BaseModel):
    ...
    def Valid_on_testsetdenoise3D(self, level):
        datamin, datamax = 0, 100  # 0.1, 99.90  #
        testset = 'Denoising_Planaria'
        spath = './models/epoch200/my_model/%s/result_valid/Norm_0-100/%s/' % (testset, level)
        ...
        for i in range(imnum):
            ...
            for ti in range(testnum):
                ...
                restored = normalize(restoredraw, datamin, datamax) * 255
            ...
            imsave(spath + name + '-ZYX_noNorm_noUint8.tif', restoredraw)
            psm, ssmm = compute_psnr_and_ssim(mean, normalize(y, datamin, datamax) * 255)
            print('Image - ', name, '- PSNR/SSIM of mean test = ', psm, ssmm)
            meanpsnr += psm
            meanssim += ssmm
