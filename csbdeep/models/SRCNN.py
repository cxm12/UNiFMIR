...
class SRCNN(BaseModel):    
    def __init__(self, config, testset, name=None, basedir='.', act='relu', train=True, method='srcnn'):
        """See class docstring."""
        self.act = act
        self.method = method
        self.testset = testset
        super(SRCNN, self).__init__(config=config, name=name, basedir=basedir)
        if train:
            trainstart = self.finetune_and_load_weights(prefer='_ep')
        else:
            trainstart = self.finetune_and_load_weights(prefer='_best')  #
        if trainstart:
            self.startepoch = 0
    ...
    def _build(self):
        print('!!! model SRCNN !!!')
        return nets.srcnn(n_channel_out=self.config.n_channel_out, activation=self.act)(self.config.unet_input_shape)
    ...
    def train(self, X, Y, validation_data, epochs=None, steps_per_epoch=None):
        ...
        if epochs != 0:
            ...
            for i in range(self.startepoch // epoch_per_step, epochs // epoch_per_step):
                ...
                meanps, meanss = self.Valid_on_testsetSR(validation_data, self.testset, imnum=5)
                if maxps < meanps:
                    maxps = meanps
                    maxss = meanss
    ...
    def Valid_on_testsetSR(self, validation_data, testset, imnum=100):  # BioSR
        self.testset = testset
        ...
        for noise in noiselst:
            ...
            meandf = 0
            if noise == 0:
                savepath = './models/epoch200/%s_%s/%s/result/AllT%d/' % (self.method, self.act, self.testset, testnum)
                # TrainF-actin/
            else:
                h, w, c = xlst[0].shape
                savepath = './models/epoch200/%s_%s/%s/result/N%f/AllT%d/' % (self.method,
                self.act, self.testset, noise, testnum)  # TrainF-actin/
                
            os.makedirs(savepath, exist_ok=True)
            for i in range(imnum):  # 6):  # 6,  5
                y = ylst[i]
                x = xlst[i]
                axes = 'YXC'
                ...
                print(f'Image - {name}- PSNR/SSIM{psm, ssmm}')
                ...
                meandf += np.mean(Meandf)
                del resultlst
            file = open(savepath + "Psnrssimmse100.txt", 'w')
            file.write('\n PSNR \n' + str(psnrlst) + '\n SSIM \n' + str(ssimlst) + '\n MSE \n' + str(
                mselst))
            file.close()
            print(psnrlst, '\n', ssimlst)
            print('%d image, Mean DF of Testset %s is %8f' % (imnum, self.testset, meandf / imnum))
