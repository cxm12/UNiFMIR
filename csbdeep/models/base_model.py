...
class BaseModel(object):
    ...
    def __init__(self, config, name=None, basedir='.', modeltype=''):
        """See class docstring."""
    ...
    @suppress_without_basedir(warn=False)
    def _find_and_load_weights(self, prefer='ep200'):  # best
        ...
        print("Loading network weights from '%s/%s'." % (self.logdir, weights_chosen.name))
    ...
    def load_weights(self, name='weights_best.h5'):
        ...
        print('******** Load weights from ', str(self.logdir/name), '**********')
        self.keras_model.load_weights(str(self.logdir/name), by_name=True)

    def _checkpoint_callbacks(self):  # 每个epoch后保存模型
        callbacks = []
        # 在model.fit添加callbacks=[checkpoint=ModelCheckpoint(...)]实现回调
    ...
    def _training_finished(self):
        if self.basedir is not None:
            if self.config.train_checkpoint_last is not None:  # weights_last.h5
                self.keras_model.save_weights(str(self.logdir / self.config.train_checkpoint_last))
            if self.config.train_checkpoint is not None:  # weights_best.h5
                print()
                self._find_and_load_weights(self.config.train_checkpoint)
            if self.config.train_checkpoint_epoch is not None:  # weights_now.h5
                try:
                    # remove temporary weights
                    (self.logdir / self.config.train_checkpoint_epoch).unlink()
                    print('training_finished and Save to', str(self.logdir / self.config.train_checkpoint_epoch))
