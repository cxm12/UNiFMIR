...
from .care_standardDropOut import CAREDropOut
from .Feedback_DropOut import FeedbackDropOut
from .care_standardDropOutdiscriminator import CAREDropOutDis
...
from ..internals.predict import predict_tiled, Progress, total_n_tiles
...
class IsotropicCAREDropOut(CAREDropOut):
    """CARE DropOut network for isotropic image reconstruction.
    """

    def predict(self, img, axes, factor, normalizer=PercentileNormalizer(), resizer=PadAndCropResizer(), batch_size=8, n_tiles=None):
        # return self._predict_mean_and_scaletile(img, axes, factor, normalizer, resizer, batch_size, n_tiles=n_tiles)[0], self._predict_mean_and_scale(img, axes, factor, normalizer, resizer, batch_size)[2]
        return self._predict_mean_and_scale(img, axes, factor, normalizer, resizer, batch_size)[0], self._predict_mean_and_scale(img, axes, factor, normalizer, resizer, batch_size)[2]
    ...
    def _predict_mean_and_scale(self, img, axes, factor, normalizer, resizer, batch_size):
        ...
        return mean, scale, x_scaled

    def _predict_mean_and_scaletile(self, img, axes, factor, normalizer, resizer, batch_size, n_tiles=None):
        normalizer, resizer = self._check_normalizer_resizer(normalizer, resizer)
        axes = axes_check_and_normalize(axes, img.ndim)
        'Z' in axes or _raise(ValueError())
        axes_tmp = 'CZ' + axes.replace('Z','').replace('C','')
        ...
        ##############################
        x_tiling_axis = tuple(axes_dict(net_axes_in)[a] for a in tiling_axes)
        ...
        _permute_axes_n_tiles = self._make_permute_axes(axes, net_axes_in)
        ...
        while not done:
            try:
                u_rot1 = predict_tiled(self.keras_model, x_rot1, axes_in=axes_predict, n_tiles=n_tiles,
                             block_sizes=net_axes_in_div_by, tile_overlaps=net_axes_in_overlaps, pbar=progress)
                done = True
                progress.close()
        ...
        while not done:
            try:
                u_rot2 = predict_tiled(self.keras_model, x_rot2, axes_in=axes_predict, n_tiles=n_tiles,
                             block_sizes=net_axes_in_div_by, tile_overlaps=net_axes_in_overlaps, pbar=progress)
                done = True
                progress.close()
            except tf.errors.ResourceExhaustedError:
...
class IsotropicFeedbackDropOut(FeedbackDropOut):
    """Feedback DropOut network for isotropic image reconstruction.
    """

    def predict(self, img, axes, factor=10.2, normalizer=PercentileNormalizer(), resizer=PadAndCropResizer(), batch_size=8, n_tiles=None):
        """Apply neural network to raw image for isotropic reconstruction.
        ...
        """
        return self._predict_mean_and_scale(img, axes, factor, normalizer, resizer, batch_size)[0]
        # return self._predict_mean_and_scaletile(img, axes, factor, normalizer, resizer, n_tiles)[0]

    def _predict_mean_and_scaletile(self, img, axes, factor, normalizer, resizer, n_tiles=None):
        normalizer, resizer = self._check_normalizer_resizer(normalizer, resizer)
        axes = axes_check_and_normalize(axes, img.ndim)
        'Z' in axes or _raise(ValueError())
        axes_tmp = 'CZ' + axes.replace('Z', '').replace('C', '')
        ...
        while not done:
            try:
                u_rot1 = predict_tiled(self.keras_model, x_rot1, axes_in=axes_predict, n_tiles=n_tiles,
                                       block_sizes=net_axes_in_div_by, tile_overlaps=net_axes_in_overlaps,
                                       pbar=progress)
                done = True
                progress.close()
            except tf.errors.ResourceExhaustedError:
...
class IsotropicCAREDropOutDis(CAREDropOutDis):
    """CARE DropOutDis network for isotropic image reconstruction.
    """
