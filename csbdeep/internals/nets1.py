...
Input, Conv2D, Conv3D, MaxPooling2D, UpSampling2D, MaxPooling3D, UpSampling3D, Activation, Lambda, Add, Concatenate, BatchNormalization, Dropout \
    = keras_import('layers', 'Input', 'Conv2D', 'Conv3D','MaxPooling2D', 'UpSampling2D', 'MaxPooling3D', 'UpSampling3D', 'Activation', 'Lambda', 'Add', 'Concatenate', 'BatchNormalization', 'Dropout')
Model = keras_import('models', 'Model')
from .blocks import unet_block, unet_blockdropout, conv_block2, conv_block3
import re
...
def common_unetdropout(n_dim=2, n_depth=1, kern_size=3, n_first=16, n_channel_out=1, residual=True, prob_out=False, last_activation='linear',modeltype=''):
    """Construct a common CARE neural net based on U-Net [1]_ and residual learning [2]_ to be used for image restoration/enhancement.
    ...
    """
    def _build_this(input_shape):
        return custom_unetdropout(input_shape, last_activation, n_depth, n_first, (kern_size,)*n_dim, pool_size=(2,)*n_dim, n_channel_out=n_channel_out, residual=residual, prob_out=prob_out, dropouttype=modeltype)
    return _build_this
...
def common_fbdropout(n_dim=2, kern_size=3, n_first=16, n_channel_out=1, residual=True,
    ...
    def _build_this(input_shape):
        return custom_fbdropout(input_shape, last_activation, n_first, (kern_size,) * n_dim,
                                n_channel_out=n_channel_out, residual=residual)
...
def custom_fbdropout(input_shape,
    ...
    for i in range(step):
        ...
        # print('dropout fcat.shape', fcat.shape)
    ...
    fd = conv(np.int32(np.ceil((n_filter_base * 4) // dropout)), (3,) * n_dim, padding=border_mode,
...
###  ------------------ SRCNN SISR --------------------- ###
def common_srcnndropout(n_dim=2, kern_size=3, n_channel_out=1,
