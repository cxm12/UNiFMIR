...
from ..utils.tf import keras_import, keras, tf
...
def common_fbUncer(n_dim=2, kern_size=3, n_first=16, n_channel_out=1, residual=True,
                     last_activation='linear', isdropout=True, step=3, scale=1):
    def _build_this(input_shape):
        return custom_fbUncer(input_shape, last_activation, n_first, (kern_size,) * n_dim,
                                n_channel_out=n_channel_out, residual=residual, isdropout=isdropout, step=step,
                                scale=scale)
    
    return _build_this


def custom_fbUncer(input_shape,
                     last_activation='linear',
                     n_filter_base=16,
                     kernel_size=(3, 3, 3),
                     dropout=0.7,
    ...
    f = conv(n_filter_base // 4, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
             name='conv0')(input)
    # sub_layer = Lambda(lambda x: tf.space_to_depth(x, 2))
    # f = sub_layer(inputs=f)
    fin = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
               name='convin')(f)
    ...
    for i in range(step):
        if i == 0:
            f0 = Concatenate(axis=channel_axis)([fin, fin])
        f1 = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                  name='conv1_%d' % i)(f0)
        up1 = f1  # upsampling(2)(f1)  #
        fup1 = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                    name='conu1_%d' % i)(up1)
        dn1 = fup1  # pooling(2, name="max_%d" % i)(fup1)  #
        fdn1 = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                    name='convd1_%d' % i)(dn1)
        
        up2 = Concatenate(axis=channel_axis)([fdn1, f1])  # upsampling(2)(Concatenate(axis=channel_axis)([fdn1, f1]))
        fup2 = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                    name='conu2_%d' % i)(up2)
        dn2 = Concatenate(axis=channel_axis)(
            [fup2, fup1])  # pooling(2, name="max2_%d" % i)(Concatenate(axis=channel_axis)([fup2, fup1]))
        fdn2 = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                    name='cond2_%d' % i)(dn2)
        
        fcat = Concatenate(axis=channel_axis)([fdn2, fdn1])
        fcat = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                    name='con2_%d' % i)(fcat)  # np.int32(n_filter_base//dropout)

        funcer = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                    name='con2un_%d' % i)(fcat)
        if scale != 1:
            funcer = upsampling(scale)(funcer)
    ...
    if isdropout:
        fd = conv(np.int32(np.ceil((n_filter_base * 4) // 0.5)), (3,) * n_dim, padding=border_mode,
                  kernel_initializer=init, activation='relu', name='concat')(fstep)
        if droptrain:
            fd = Dropout(dropout)(fd, training=istrain)
        else:
            fd = Dropout(dropout)(fd)
    else:
        fd = conv(n_filter_base, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu',
                  name='concat')(fstep)
        # fd = conv(n_filter_base * 4, (3,) * n_dim, padding=border_mode, kernel_initializer=init, activation='relu', name='concat')(fstep)
    # sub_layer = Lambda(lambda x: tf.depth_to_space(x, 2))
    # fd = sub_layer(inputs=fd)
    ...
    return Model(inputs=input, outputs=[final, theta])
...
def srcnn(n_channel_out, activation):
    def _build_this(input_shape):
        keras.backend.set_learning_phase(1)
        n_dim = 2  # len(kernel_size)
        conv = Conv2D if n_dim == 2 else Conv3D
        border_mode = "same"
        init = "glorot_uniform"
        ...
        ## define model :
        input = Input(input_shape, name="input")
        # bic = UpSampling2D(size=(2, 2), data_format=None, interpolation='bilinear')(input)  # tf.keras.layers.
        bic = UpSampling2D(2)(input)
        ...
        if activation == 'relu':
            ...
            f = conv(64, (9,) * n_dim, padding=border_mode, kernel_initializer=init, activation='linear',
                     name='conv0')(bic)
            f2 = conv(32, (1,) * n_dim, padding=border_mode, kernel_initializer=init, activation='linear',
                      name='convin')(f)
            # keras.activations.linear(f)
        elif activation == 'tanh':
            f = conv(64, (9,) * n_dim, padding=border_mode, kernel_initializer=init, activation='tanh',
                     name='conv0')(bic)
            f2 = conv(32, (1,) * n_dim, padding=border_mode, kernel_initializer=init, activation='tanh',
                      name='convin')(f)
        elif activation == 'our1':  # 'our2'
            f = conv(64, (9,) * n_dim, padding=border_mode, kernel_initializer=init, activation=None,
                     name='conv0')(bic)
        ...
        elif activation == 'our11':
            f = conv(64, (9,) * n_dim, padding=border_mode, kernel_initializer=init, activation=None,
                     name='conv0')(bic)
            f = act_func(f, delta_T=0.08, sat_I=7.268, ns_T=0.572)
            f2 = conv(32, (1,) * n_dim, padding=border_mode, kernel_initializer=init, activation=None,
                      name='convin')(f)
            f2 = act_func(f2, delta_T=0.08, sat_I=7.268, ns_T=0.572)
        elif activation == 'our4':
            f = conv(64, (9,) * n_dim, padding=border_mode, kernel_initializer=init, activation=None,
                     name='conv0')(bic)
            f = act_func(f, delta_T=0.237, sat_I=9.527, ns_T=0.963)
            f2 = conv(32, (1,) * n_dim, padding=border_mode, kernel_initializer=init, activation=None,
                      name='convin')(f)
            f2 = act_func(f2, delta_T=0.237, sat_I=9.527, ns_T=0.963)
