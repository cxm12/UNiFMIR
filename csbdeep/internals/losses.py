...
def loss_discriminator(mean=True):
    R = _mean_or_not(mean)
    if backend_channels_last():
        def CE(y_true, y_pred):
            n = K.shape(y_true)[-1]
            gt = categorical_crossentropy(np.ones_like(y_true), y_true, from_logits=True)
            sr = categorical_crossentropy(np.zeros_like(y_pred[...,:n]), y_pred[...,:n], from_logits=True)
            return R(gt+sr)
        return CE
    else:
        def CE(y_true, y_pred):
            ...
            gt = categorical_crossentropy(np.ones_like(y_true), y_true, from_logits=True)
            sr = categorical_crossentropy(np.zeros_like(y_pred[:,:n,...]), y_pred[..., :n], from_logits=True)
            return R(gt+sr)  # R(K.square(y_pred[:,:n,...] - y_true))
        return CE


def loss_generator(mean=True):
    R = _mean_or_not(mean)
    if backend_channels_last():
        def CE(y_pred):
            n = K.shape(y_pred)[-1]
            sr = categorical_crossentropy(np.ones_like(y_pred[:,:n,...]), y_pred[..., :n], from_logits=True)
            return R(sr)
        return CE
    else:
        def CE(y_pred):
