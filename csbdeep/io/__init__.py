...
from ..utils import _raise, axes_check_and_normalize, axes_dict, move_image_axes, move_channel_for_backend, \
...
def load_training_data(file, validation_split=0, axes=None, n_images=None, verbose=False):
    ...
    f = np.load(file)
    X, Y = f['X'], f['Y']
    # Isotropic_Drosophila: [44744, 1, 128, 128]   Isotropic_Retina[25391, 2, 128, 128] Isotropic_Liver[3872, 1, 128, 128]
    # Microtubules: (19800, 1, 256, 256)
    # print(np.min(X), np.max(X))  # -0.58878505 87.93965
    # print(np.min(Y), np.max(Y))  # 0.00067468104 4.548696
    print(Y.ndim, Y.shape)  # 5 (17900, 1, 16, 64, 64)  # 4 (4914, 1, 128, 128) # Isotropic_Drosophila (44744, 1, 128, 128)
    if axes is None:
        axes = f['axes']  # 'SCZYX'
