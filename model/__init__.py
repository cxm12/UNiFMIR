...
class Model(nn.Module):
    ...
    def forward(self, x, idx_scale):
        ...
        if self.chop and not self.training and self.args.test_only:
            if 'proj' in self.args.model or self.args.task == 4:
                return self.forward_chopProj(x)
            elif '2t3' in self.args.model or self.args.task == 5:
    ...
    def forward_chopProj(self, x, shave=10, min_size=120000):
        ...
        if w_size * h_size < min_size:
            ...
            sr_list = [
                self.forward_chopProj(patch, shave=shave, min_size=min_size) for patch in lr_list]
    ...
    def forward_chop2to3(self, x, shave=2, min_size=120000):
        scale = 11
        n_GPUs = min(self.n_GPUs, 4)
        b, c, h, w = x.size()  # [1, 121, 16, 16]
        h_half, w_half = h // 2, w // 2
        
        h_size, w_size = h_half + shave, w_half + shave
      
        h_size += 8-h_size % 8  # h_size += 4-h_size % 4  # [20, 24]
        w_size += 8-w_size % 8
