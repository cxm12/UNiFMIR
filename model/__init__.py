...
class Model(nn.Module):
    ...
    def forward_chopProj(self, x, tsk, shave=10, min_size=120000):
        ...
        if w_size * h_size < min_size:
            ...
            for i in range(0, 4, n_GPUs):
                lr_batch = torch.cat(lr_list[i:(i + n_GPUs)], dim=0)
                print('Proj Output')
                sr_batch1, sr_batch = self.model(lr_batch, tsk)
                # sr_batch1, sr_batch, _ = self.model(lr_batch, tsk)
                sr_lists1.extend(sr_batch1.chunk(n_GPUs, dim=0))
                sr_list.extend(sr_batch.chunk(n_GPUs, dim=0))
        else:
            sr_lists1 = [self.forward_chopProj(patch, tsk, shave=shave, min_size=min_size)[0] for patch in lr_list]
            sr_list = [
                self.forward_chopProj(patch, tsk, shave=shave, min_size=min_size)[1] for patch in lr_list]
        ...
        outputs1[:, :, h_half:h, w_half:w] = sr_lists1[3][:, :, (h_size - h + h_half):h_size,
    ...
    def forward_chop2to3(self, x, tsk, shave=2, min_size=120000):
        ...
        if w_size * h_size < min_size:
            ...
            sr_lists1 = [self.forward_chop2to3(patch, tsk, shave=shave, min_size=min_size)[0] for patch in lr_list]
            sr_list = [self.forward_chop2to3(patch, tsk, shave=shave, min_size=min_size)[1] for patch in lr_list]
